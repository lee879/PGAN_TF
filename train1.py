...
from model_net import initModel_G,initModel_D,G_Backbone,D_Backbone
import config
...
def gradient_penalty(discriminator,batch_xy,fake_image,alpha): # wgan主要的贡献
    ...
    with tf.GradientTape() as tape:
        ...
        d_interplate_logits = discriminator(interplate,alpha)
        d_interplate_logits_loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=d_interplate_logits,
                                                                           labels=tf.ones_like(d_interplate_logits))
    grads = tape.gradient(d_interplate_logits_loss ,interplate)
    #grads[b,h,w,c]
...
def d_loss_fn(init_G, init_D, real_img, laten,alpha):
    fake = init_G(laten,alpha)
    critic_real = init_D(real_img,alpha)
    critic_fake = init_D(fake,alpha)
    gp = gradient_penalty(init_D,real_img,fake,alpha)
    loss = (
...
def g_loss_fn(init_G, init_D, real_img, laten,alpha):
    fake_img = init_G(laten,alpha)
    d_fake_logits = init_D(fake_img,alpha)
...
def main():
    ...
    model_G = initModel_G(config.Factors_G_Channels[step],name=name_number)  # 加载初始模型,也就是最简单的progan生产器的第一层模型
    model_D = initModel_D(config.Factors_D_Channels[8 - step],name=name_number)  # 加载初始模型，也就是 progan鉴别器 的第一层模型
    for num_epochs in config.PROGRESSIVE_EPOCHS[step:]:
        ...
        for epoch in range(num_epochs):
            ...
            alpha = 0.6#special_cosine(epoch,num_epochs)
            ...
            with tf.GradientTape() as tape:
                d_loss, gp = d_loss_fn(model_G, model_D, real_img, laten,alpha)
            grads = tape.gradient(d_loss,model_D.trainable_variables)
            tf.optimizers.Adam(learning_rate=0.001, beta_1=0.5,beta_2=0.99).apply_gradients(
                zip(grads, model_D.trainable_variables))
            #生产器的

            with tf.GradientTape() as tape:
                g_loss, fake_img = g_loss_fn(model_G, model_D, real_img, laten,alpha)
            grads = tape.gradient(g_loss, model_G.trainable_variables)
            tf.optimizers.Adam(learning_rate=0.001, beta_1=0.5,beta_2=0.99).apply_gradients(zip(grads, model_G.trainable_variables))
